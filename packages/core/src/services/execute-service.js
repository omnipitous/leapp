...
class ExecuteService {
    constructor(nativeService, repository, logService) {
     ...
     */
    async execute(command, env, maskOutputLog) {
    ...
    openTerminal(command, env, macOsTerminalType) {
        ...
        if (this.nativeService.process.platform === "darwin") {
            ...
            const path = this.nativeService.os.homedir() + "/" + constants_1.constants.ssmSourceFileDestination;
            if (terminalType === constants_1.constants.macOsTerminal) {
                return this.execute(`osascript -e 'if application "Terminal" is running then\n
                    ...
                    \t\tactivate\n
                    \tend tell\n
                    end if'`, Object.assign(newEnv, env));
            }
            else if (terminalType === constants_1.constants.macOsIterm2) {
                return this.execute(`osascript -e 'if application "iTerm" is running then\n
            ...
            else if (terminalType === constants_1.constants.macOsWarp) {
                return this.execute(`osascript -e 'if application "Warp" is running then\n
    ...
    openTerminalFromPlugin(command, env) {
        ...
        const pluginEnvPath = this.nativeService.os.homedir() + "/" + constants_1.constants.pluginEnvFileDestination;
        if (this.nativeService.process.platform === "darwin") {
            return this.execute(`osascript -e 'if application "Terminal" is running then\n
                    \ttell application "Terminal"\n
    ...
    async exec(execFn, command, env = undefined, maskOutputLog = false) {
        ...
        return new Promise((resolve, reject) => {
            execFn(command, { env, name: "Leapp", timeout: 60000 }, (err, stdout, stderr) => {
                ...
                this.logService.log(new log_service_1.LoggedEntry("execute from Leapp\ninfo:" + JSON.stringify(info, undefined, 4), this, log_service_1.LogLevel.info, false));
                if (err) {
                    err.stdout = stdout;
                    err.stderr = stderr;
                    reject(err);
